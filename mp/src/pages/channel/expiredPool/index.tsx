        ...
        list.map((it) => (
          <View key={it._id} className="expired-item">
            <View className="expired-item__row">
              <Text className="expired-item__name">{it.customerId && it.customerId.name}</Text>
              <Tag type={(it.customerId && it.customerId.source) === "PERSONAL_REFERRAL" ? "primary" : "warning"}>
                {(it.customerId && it.customerId.source) === "PERSONAL_REFERRAL" ? "个人" : "公司"}
              </Tag>
            </View>
            <View className="expired-item__row">
              <Text>原推荐人: {(it.customerId && it.customerId.channelReferrer && it.customerId.channelReferrer.referrerName) || "-"}</Text>
